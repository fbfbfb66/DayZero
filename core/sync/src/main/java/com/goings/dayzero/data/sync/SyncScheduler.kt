...
import com.goings.dayzero.data.sync.media.MediaPullCoordinator
import com.goings.dayzero.data.sync.media.MediaPullResult
...
class InProcessSyncScheduler(
    ...
    private val chatPullCoordinator: com.goings.dayzero.data.sync.chat.ChatPullCoordinator? = null,
    private val chatPullHealthStateStore: com.goings.dayzero.data.sync.chat.ChatPullHealthStateStore? = null,
    ...
    private val mediaPullCoordinator: MediaPullCoordinator? = null
) : SyncScheduler {
            ...
            val job = scope.launch {
                try {
                    ...
                    if (pullMode != null) {
                        ...
                        when (chatResult) {
                            is com.goings.dayzero.data.sync.chat.ChatPullResult.Success -> {
                                chatPullHealthStateStore?.markCompleted()
                            }
                            is com.goings.dayzero.data.sync.chat.ChatPullResult.ConversationRetryableFailure -> {
                                chatPullHealthStateStore?.markRetryableFailure(chatResult.reason)
                            }
                            is com.goings.dayzero.data.sync.chat.ChatPullResult.ConversationFatalFailure -> {
                                chatPullHealthStateStore?.markFatalFailure(chatResult.reason)
                            }
                            is com.goings.dayzero.data.sync.chat.ChatPullResult.MessageRetryableFailure -> {
                                chatPullHealthStateStore?.markRetryableFailure(chatResult.reason)
                            }
                            is com.goings.dayzero.data.sync.chat.ChatPullResult.MessageFatalFailure -> {
                                chatPullHealthStateStore?.markFatalFailure(chatResult.reason)
                            }
                            is com.goings.dayzero.data.sync.chat.ChatPullResult.Skipped,
                            com.goings.dayzero.data.sync.chat.ChatPullResult.SkippedAlreadyRunning,
                            null -> Unit
                        }

                        Log.d("DayZeroMediaPull", "scheduler step media pull reason=$reason")
                        val mediaResult = mediaPullCoordinator?.pullMedia()
                        if (mediaResult is MediaPullResult.Success && mediaResult.stats.insertedCount > 0) {
                            // New remote assets were inserted as REMOTE_PENDING with DOWNLOAD tasks
                            // enqueued; flush them so bytes are fetched promptly.
                            Log.d("DayZeroMediaPull", "scheduler flushing ${mediaResult.stats.insertedCount} media downloads reason=$reason")
                            syncCoordinator?.runOnce()
                        }
                    }
