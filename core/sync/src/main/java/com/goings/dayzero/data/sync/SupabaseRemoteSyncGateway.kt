...
import com.goings.dayzero.data.remote.mapper.RemoteChatSyncMapper
...
import com.goings.dayzero.domain.model.sync.ChatSyncConversationSnapshot
import com.goings.dayzero.domain.model.sync.ChatSyncMessageSnapshot
...
class SupabaseRemoteSyncGateway(
    ...
    override suspend fun upsertMediaAsset(payload: SyncPayload): RemoteSyncResult {
        ...
        val binaryStore = mediaBinaryStore ?: return RemoteSyncResult.Skipped("media_binary_store_unavailable")
        val dao = mediaAssetDao ?: return RemoteSyncResult.Skipped("media_dao_unavailable")
        val mediaId = payload.clientId()
        val media = dao.getById(mediaId)
            ?: return RemoteSyncResult.FatalFailure("media_row_missing:$mediaId")
        val session = sessionProvider.currentSessionOrNull() ?: return sessionUnavailableSyncResult()

        // Soft-deleted asset: push the tombstone metadata only, keep bytes in Storage.
        if (media.deletedAt != null) {
            return upsertMediaMetadata(media, media.remoteMasterPath, media.remoteThumbnailPath)
        }

        val masterRel = media.masterRelativePath
            ?: return RemoteSyncResult.FatalFailure("media_master_path_missing:$mediaId")
        val masterBytes = binaryStore.readBytes(masterRel)
            ?: return RemoteSyncResult.FatalFailure("media_master_file_missing:$mediaId")
        val thumbBytes = media.thumbnailRelativePath?.let { binaryStore.readBytes(it) }

        val masterObjectPath = "${session.userId}/$mediaId/master.jpg"
        val thumbObjectPath = "${session.userId}/$mediaId/thumb.jpg"

        // Bytes before metadata: any row a recipient later pulls always has downloadable objects.
        uploadStorageObject(session, masterObjectPath, masterBytes)
            .let { if (it !is RemoteSyncResult.Success) return it }
        if (thumbBytes != null) {
            uploadStorageObject(session, thumbObjectPath, thumbBytes)
                .let { if (it !is RemoteSyncResult.Success) return it }
        }
        val effectiveThumbPath = if (thumbBytes != null) thumbObjectPath else null

        upsertMediaMetadata(media, masterObjectPath, effectiveThumbPath)
            .let { if (it !is RemoteSyncResult.Success) return it }

        dao.markUploaded(mediaId, masterObjectPath, effectiveThumbPath, System.currentTimeMillis())
        return RemoteSyncResult.Success
    }

    override suspend fun downloadMediaAsset(payload: SyncPayload): RemoteSyncResult {
        if (!isConfigured) return RemoteSyncResult.Skipped("supabase_not_configured")
        val binaryStore = mediaBinaryStore ?: return RemoteSyncResult.Skipped("media_binary_store_unavailable")
        val dao = mediaAssetDao ?: return RemoteSyncResult.Skipped("media_dao_unavailable")
        val mediaId = payload.clientId()
        ...
        if (media.deletedAt != null) return RemoteSyncResult.Success
        if (media.remoteSyncState == MediaRemoteSyncState.DOWNLOADED.name &&
            media.masterRelativePath != null
        ) {
            return RemoteSyncResult.Success
        }

        val session = sessionProvider.currentSessionOrNull() ?: return sessionUnavailableSyncResult()
        val masterObjectPath = media.remoteMasterPath
            ?: return RemoteSyncResult.FatalFailure("media_remote_master_missing:$mediaId")

        val (masterBytes, masterError) = downloadStorageObject(session, masterObjectPath)
        if (masterBytes == null) return masterError ?: RemoteSyncResult.RetryableFailure("media_download_failed")
        val thumbBytes = media.remoteThumbnailPath?.let { downloadStorageObject(session, it).first }

        val written = binaryStore.writeDownloaded(mediaId, masterBytes, thumbBytes)
        dao.markDownloaded(
            id = mediaId,
            masterRelativePath = written.masterRelativePath,
            thumbnailRelativePath = written.thumbnailRelativePath,
            mimeType = null,
            width = null,
            height = null,
            byteSize = null,
    ...
    private suspend fun upsertMediaMetadata(
        media: MediaAssetEntity,
        masterObjectPath: String?,
        thumbnailObjectPath: String?
    ): RemoteSyncResult {
        return upsertById(
            entityType = MEDIA_ASSET_ENTITY_TYPE,
            tableName = "media_assets",
            clientId = media.id,
            body = mediaAssetBody(media, masterObjectPath, thumbnailObjectPath)
        )
    }

    private fun mediaAssetBody(
        media: MediaAssetEntity,
        masterObjectPath: String?,
        thumbnailObjectPath: String?
    ): JSONObject {
        return JSONObject()
            .put("id", media.id)
            .put("conversation_id", media.conversationId)
            .putNullable("source_message_id", media.sourceMessageId)
            .put("conversation_order", media.conversationOrder)
            .putNullable("master_object_path", masterObjectPath)
            .putNullable("thumbnail_object_path", thumbnailObjectPath)
            .putNullable("mime_type", media.mimeType)
            .putNullable("width", media.width)
            .putNullable("height", media.height)
            .putNullable("byte_size", media.byteSize)
            .putNullable("sha256", media.sha256)
            .put("source", media.source)
            .put("created_at", Instant.ofEpochMilli(media.createdAt).toString())
            .put("updated_at", Instant.ofEpochMilli(media.updatedAt).toString())
            .put("deleted_at", media.deletedAt?.let { Instant.ofEpochMilli(it).toString() } ?: JSONObject.NULL)
    ...
    private suspend fun uploadStorageObject(
        ...
        val request = Request.Builder()
            .url("${storageUrl()}object/$storageBucket/$objectPath")
            .header("apikey", anonKey)
            .header("Authorization", "Bearer ${session.accessToken}")
            .header("Content-Type", JPEG_CONTENT_TYPE)
            .header("x-upsert", "true")
            .post(bytes.toRequestBody(JPEG_MEDIA_TYPE))
            .build()
        Log.d("DayZeroRemote", "storage upload start object=$objectPath size=${bytes.size}")
        return executeRequest(request, entityType = MEDIA_ASSET_ENTITY_TYPE, clientId = objectPath)
    }

    private suspend fun downloadStorageObject(
        session: SupabaseAuthSession,
        objectPath: String
    ): Pair<ByteArray?, RemoteSyncResult?> = withContext(Dispatchers.IO) {
        val request = Request.Builder()
            .url("${storageUrl()}object/$storageBucket/$objectPath")
            .header("apikey", anonKey)
            .header("Authorization", "Bearer ${session.accessToken}")
        ...
        try {
            okHttpClient.newCall(request).execute().use { response ->
                when {
                    response.isSuccessful -> Pair(response.body?.bytes(), null)
                    response.code in RETRYABLE_STATUS_CODES ->
                        Pair(null, RemoteSyncResult.RetryableFailure("http_${response.code}"))
                    response.code in FATAL_STATUS_CODES ->
                        Pair(null, RemoteSyncResult.FatalFailure("http_${response.code}"))
                    else -> Pair(null, RemoteSyncResult.RetryableFailure("http_${response.code}"))
                }
            }
        } catch (e: Exception) {
            Pair(null, RemoteSyncResult.RetryableFailure(e.message ?: e::class.java.simpleName))
        }
    }

    private fun storageUrl(): String = "${normalizedUrl()}storage/v1/"
    ...
    private companion object {
        private const val MEDIA_ASSET_ENTITY_TYPE = "media_asset"
        private const val JPEG_CONTENT_TYPE = "image/jpeg"
        private val JSON_MEDIA_TYPE = "application/json".toMediaType()
        private val JPEG_MEDIA_TYPE = JPEG_CONTENT_TYPE.toMediaType()
