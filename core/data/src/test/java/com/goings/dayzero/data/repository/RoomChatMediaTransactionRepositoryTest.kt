...
class RoomChatMediaTransactionRepositoryTest {
        ...
        assertTrue(
            tasks.any {
                it.entityType == com.goings.dayzero.data.sync.media.MediaSyncQueueContract.ENTITY_MEDIA_ASSET &&
                    it.operation == com.goings.dayzero.data.sync.media.MediaSyncQueueContract.OP_UPSERT_MEDIA_ASSET &&
                    it.entityLocalId == mediaIds.single()
            }
        )
    ...
    fun sourceMediaIdsOrderIsPreservedInContentJson() = runBlocking {
        ...
        val domain = com.goings.dayzero.data.local.mapper.AiChatMessageMapper().toDomain(message)
    ...
    fun partiallyBoundExistingMessageReturnsConflict() = runBlocking {
        ...
        database.aiChatMessageDao().insertMessage(
            com.goings.dayzero.data.local.mapper.AiChatMessageMapper().toEntity(
                AiChatMessage(
                    id = userMessageId,
                    conversationId = conversationId,
                    role = ChatRole.User,
    ...
    private suspend fun insertPlaceholder(userMessageId: String, conversationId: String, createdAt: Long) {
        database.aiChatMessageDao().insertMessage(
            AiChatMessageEntity(
                ...
                relatedDraftId = null,
                messageType = com.goings.dayzero.domain.model.ai.ChatMessageType.Text.name,
                contentJson = null,
                assistantCardsJson = null,
                suggestedRepliesJson = null,
                updatedAt = createdAt,
    ...
    private class MismatchMediaDao(private val delegate: MediaAssetDao) : MediaAssetDao {
        ...
        override suspend fun findAssetsNeedingUpload(limit: Int): List<MediaAssetEntity> =
            delegate.findAssetsNeedingUpload(limit)

        override suspend fun findAssetsNeedingDownload(limit: Int): List<MediaAssetEntity> =
            delegate.findAssetsNeedingDownload(limit)
        ...
        override suspend fun markUploaded(
            ...
            updatedAt: Long
        ): Int = delegate.markUploaded(id, remoteMasterPath, remoteThumbnailPath, updatedAt)
        ...
        override suspend fun applyRemoteTombstone(
            ...
            deletedAt: Long,
            updatedAt: Long
        ): Int = delegate.applyRemoteTombstone(id, deletedAt, updatedAt)
    }
