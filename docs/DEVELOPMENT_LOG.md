...
- 严格冻结：`src/shared/assistant_upstream_timeout.ts`、`src/shared/prompt.ts`、`src/shared/normalization.ts`、`src/shared/assistant_vision.ts`、`src/shared/explicit_photo_meal_assignment.ts` 哈希未变化。
- 认证加固：
  - `src/config.ts` 新增 `APP_ENV`、`SUPABASE_JWKS_URL`、`SUPABASE_ISSUER`、`SUPABASE_AUDIENCE`；保留 `SUPABASE_URL`/`SUPABASE_JWT_AUDIENCE` 兼容，新变量优先。
  - `APP_ENV=production` 且 `ENABLE_AUTH=false` 时启动 fail-fast。
  - `src/auth.ts` 强制 ES256、校验 `kid`/`exp`/`sub`/`iss`/`aud`、未知 `kid` 触发一次 JWKS 刷新、JWKS 不可用不绕过认证。
  - 认证错误统一返回固定 `AuthErrorCode`，不暴露 jose 原始错误、claim 值或 Token 片段。
- 日志安全：
  - `src/logger.ts` 新增 `digestUserId()`（SHA-256 前缀），认证成功返回 `u_<hex>` 用户摘要。
  - 日志上下文额外脱敏 `sub`、`userText`、`recentMessages`、`todayRecord`、`interactionResult`、`prompt`、Base64、data URL。
  - 助手流程日志携带 `userIdDigest`，不携带原始 `sub`。
- 部署可追溯：
  - `Dockerfile` 增加 `VCS_REF`、`SOURCE_BUNDLE_SHA`、`BUILD_DATE`、`IMAGE_VERSION` 构建参数与 OCI labels。
  - 新增 `docker-compose.production.yml`：Gateway `expose: 8080`（不映射宿主机），Nginx 独占 80/443，healthcheck 使用 `/ready`，日志轮转。
  - 新增 `nginx.production.conf.template`：HTTPS 443、HTTP→HTTPS、ACME、所有新/旧路由、SSE 完整防缓冲指令、`X-Accel-Buffering no`、无跨服务器 fallback。
  - 新增 `deployment.manifest.template.yml` 与 `docs/DEPLOYMENT_AND_ROLLBACK_20260712.md`。
- 验证：Deno fmt/lint/check/test 全通过；测试数从 80 提升至 100；Docker build、Nginx config test（临时证书）、Compose config 通过；镜像 OCI labels 已验证；日志扫描未出现敏感内容。
- 明确未实施：未部署 ECS、未重启服务、未修改 Android/Supabase/Room、未调用真实 Kimi、未输出 Secret。
- 仍缺：真实域名、DNS、生产 HTTPS 证书；下一阶段为独立安全验收与受控 ECS 部署。

### Phase G2 独立安全验收与 G2-F1 修复（2026-07-12）

- **G2 初次完成状态**：`PHASE_G2_COMPLETE`；相关完成报告见 `docs/PHASE_G2_COMPLETION_REPORT_20260712.md`。
- **独立安全验收（Codex 审计）**：`NOT_ACCEPTABLE_FOR_DEPLOYMENT`。关键发现包括：
  - `src/logger.ts` 黑名单不完整，`pendingDraft`/`detail`/`message`/`error`/`cause`/`stack`/`imagePath` 等可进入日志。
  - `src/errors.ts` 与 stream handler 将 raw exception message / Kimi body 回显给客户端或写入日志。
  - `/ready` 仍依赖 legacy Supabase 配置字段，导致仅使用新 `SUPABASE_JWKS_URL`/`SUPABASE_ISSUER`/`SUPABASE_AUDIENCE` 时返回 `503`。
  - Docker healthcheck 错误使用 `/health` 而非 `/ready`。
  - JWKS fetch 无超时、无 single-flight、无失败退避。
  - 部署/回滚文档缺少真实 ACME 流程与可执行回滚命令。
  - 详细报告见 `docs/PHASE_G2_SECURITY_ACCEPTANCE_20260712.md`。
- **G2-F1 定点修复**：针对上述 Critical/High 问题完成代码与文档修复，状态 `PHASE_G2_F1_COMPLETE`：
  - `src/logger.ts` 改为显式白名单，丢弃所有对象/数组/异常内容；新增 17 个安全日志测试。
  - `src/errors.ts` 与 fallback/stream handler 使用固定 `UpstreamErrorCode` 与安全文案，不再透传 Kimi body 或 raw error message。
  - `/ready` 仅检查规范化 `supabaseJwksUrl`/`supabaseIssuer`/`supabaseAudience`。
  - Docker healthcheck 与 Compose 均改为探测 `/ready`；Nginx 依赖 Gateway `service_healthy`。
  - JWKS fetch 增加 4s 超时、全局 single-flight、未知 kid 单请求刷新、5s 失败 cooldown。
  - 新增 `.dockerignore`；生产 Compose 强制使用 digest-only image 引用。
  - `docs/DEPLOYMENT_AND_ROLLBACK_20260712.md` 补齐 ACME 首签/续期/失败不切流与可执行 v1 回滚命令。
  - 测试数从 100 提升至 127 并全部通过；Docker build、Compose config、`nginx -t`（临时证书）均通过。
  - 完成报告见 `docs/PHASE_G2_F1_COMPLETION_REPORT_20260712.md`。
- **G2-F1 独立安全复验**：`ACCEPTABLE_FOR_CONTROLLED_DEPLOYMENT`。全部 Critical/High 问题已修复，未发现新增阻断风险；冻结文件哈希与基线一致。复验报告见 `docs/PHASE_G2_F1_SECURITY_REVERIFICATION_20260712.md`。
- **仍未部署**：未执行 ECS 部署、Nginx 切换、真实证书签发；真实域名/DNS/证书仍为后续阶段前置条件。
...
- **架构清理**：将 `DailyRecord`、`MealEntry`、`FoodEntry` 等领域模型从 `DayZeroViewModel` 中提取到独立的 `com.goings.dayzero.domain.model` 包下。

- **数据解耦**：将 `createMockRecords` 提取到 `com.goings.dayzero.data.mock`，使 ViewModel 逻辑更加纯粹。
...
* `:app/src/test/java/com/goings/dayzero/VisionAssistantTurnOrchestratorTest.kt`：新增 20 个单元测试，覆盖：
...
1. **Vision 流式诊断日志**
   ...
   * 修复 `FallbackReason.isIoLike()` 中错误引用 `com.goings.dayzero.domain.model.ai.assistant.ProtocolException` 的问题，改为正确排除 `java.net.ProtocolException`。
...
## 2026-07-11 Commit 2bd958e — 照片分配编辑器与相关实现正式入库

### 背景

2026-07-10 Phase 4B-1-R 现实审计发现：照片分配编辑器源码完整且接线正确，但全部改动处于未提交工作区（`photoeditor/` 目录、UseCase、Repo、接线文件均未 commit），导致用户设备上已安装的 APK 从未包含该功能。审计结论是「第一断点在交付层，而非代码层」。

### 处置

- **未修改任何源码逻辑**：现有实现已通过审计，满足入口矩阵、来源解析、草稿不变量、保存/取消、Date Guard、陈旧 Compose 草稿防护、Viewer 复用等全部要求，禁止按「第二套编辑器」规则重写。

- **直接提交现有工作区改动**：将 2026-07-10 审计时的完整工作区状态提交为 `2bd958e Add and update feature implementations`，从而关闭交付缺口。

### 提交内容清单

- `feature/ai-record/src/main/java/com/goings/dayzero/ui/screens/photoeditor/`：

  - `PhotoAssignmentEditorScreen.kt`（沉浸式全屏编辑器 UI）

  - `PhotoAssignmentDraft.kt`（不可变分配草稿模型与纯函数操作）

  - `FanDeckMath.kt`（扇形牌堆几何/吸附计算）

  - `PhotoEditorCardResolver.kt`（卡片查找、来源图片严格配对、编辑合法性矩阵）

  - `PhotoAssignmentEditorUiState.kt`（编辑器会话状态）

- `core/domain/src/main/java/com/goings/dayzero/domain/usecase/UpdateFoodCardPhotoAssignmentsUseCase.kt`

- `core/data/src/main/java/com/goings/dayzero/data/repository/RoomFoodCardPhotoAssignmentRepository.kt`

- `core/ui/src/main/java/com/goings/dayzero/ui/components/PhotoViewerOverlay.kt`（复用型图片查看器）

- `core/ui/src/main/java/com/goings/dayzero/ui/components/PinnedPhotoStrip.kt`（安静手帐风照片条重设计）

- `core/ui/src/main/java/com/goings/dayzero/ui/components/LocalMediaThumbnail.kt`

- 接线文件：`AiRecordScreen.kt`、`AiRecordViewModel.kt`、`AssistantCardRenderer.kt`、`FoodDraftConfirmCard.kt`、`AppNavigation.kt`、`DayZeroHiltModule.kt`、`DayZeroViewModel.kt`

- Edge Function 共享模块与运行时：

  - `supabase/functions/_shared/assistant_upstream_timeout.ts`（动态上游 header 超时）

  - `supabase/functions/_shared/explicit_photo_meal_assignment.ts`（显式多餐照片分配解析器）

  - `assistant-turn-v2-stream/handler.ts` / `normalization.ts`

  - `assistant-turn-v2/handler.ts` / `normalization.ts`

- 跨模块新增/更新测试：`:app`（含 `AiRecordPhotoEditorSavePersistenceTest`、`DayZeroInteractionResultPhotoAssignmentTest`、`VisionSingleMealPhotoAssignmentProductionPathTest` 等）、`:feature:ai-record`（含 `PhotoAssignmentEditorScreenTest`、`PhotoAssignmentEditorSessionTest`、`PhotoEditorViewerReuseTest` 等）、`:core:model`、`:core:domain`、`:core:data`、`:core:sync`、`:core:ui`。

### 测试与构建结果

- `:core:model:test` PASS、`:core:domain:test` PASS、`:core:data:testDebugUnitTest` PASS、`:core:sync:testDebugUnitTest` PASS、`:core:ui:testDebugUnitTest` PASS、`:feature:ai-record:testDebugUnitTest` PASS。

- `:app:assembleDebug` BUILD SUCCESSFUL。

- `:app:testDebugUnitTest`：138 测试，**仅 2 个既有基线失败**（时区相关，Requirement §13 白名单），**无新增失败**。

- 根级 `test` 退出码 `ROOT_TEST_EXIT_CODE=1`，失败同样仅上述 2 个时区测试，无新增失败。

### 未触及边界

未修改 assistant-turn-v2 / -stream 的 AI Prompt 语义（仅超时/显式分配辅助逻辑）、Kimi/Vision 核心 timeout 常量之外的逻辑、派生图压缩参数、Supabase Schema/RLS/Storage/Auth/Secrets、Room version/Migration、`MealEntry.mediaIds`、DailyRecord 正式照片持久化、Calendar 照片、云端媒体同步。未执行 adb 安装/卸载、`pm clear`、connectedDebugAndroidTest 或真实用户数据读取。

### 结论

`READY_FOR_PHOTO_EDITOR_DEVICE_RETEST` —— 照片分配编辑器实现已正式入 Git，但**仍需用户安装新构建的 `app-debug.apk` 后在真机复测**完整流程（pending Confirm Card → 整理照片 · N 张 → 全屏编辑器 → 改分配 → 保存 → Room 卡片更新 → 再次打开仍是新分配）。**未声明** `PHASE_4B_COMPLETE` / `DEVICE_TEST_PASSED`。

## 2026-07-12 当前状态快照

- **代码库**：`fix/audit-safe-local-fixes` 分支，`working tree clean`，与 `origin/fix/audit-safe-local-fixes` 同步；最新提交 `2bd958e Add and update feature implementations`。

- **Edge Function 远端版本**：`assistant-turn-v2-stream` **v17**，`assistant-turn-v2` **v28**，均 `ACTIVE` / `verify_jwt=false`。

- **Prompt 版本**：`stream_compact_v7_deterministic_multi_meal_photo_assignment` / `compact_v8_deterministic_multi_meal_photo_assignment`。

- **Room 版本**：12。

- **测试基线**：`:app:assembleDebug` 通过；`:app:testDebugUnitTest` 与根级 `test` 仅 2 个既有时区脆弱测试失败（白名单），无新增失败。

- **待验证**：照片分配编辑器真机复测、Vision 动态超时非高峰时段真机复测。

- **未实现/未声明完成**：`PHASE_4B_COMPLETE`、正式登录/注册、Token 加密存储、MealEntry/DailyRecord/Calendar 照片持久化、多设备媒体同步、Phase 5/6 后续重构。

## 2026-07-11 跨设备照片同步（Supabase Storage · 方案 A）

### 背景
之前照片仅存本地 Room + `filesDir`，聊天同步只携带 `media.sourceMediaIds`，换手机后这些 id 指向不存在的行/文件 → 图片丢失。本次补齐「图片字节 + 元数据」的跨设备同步。

### 实现
- **本地 DB**：Room 12→13（`MIGRATION_12_13`），`media_assets` 新增 `remoteSyncState`（LOCAL_ONLY/UPLOADED/REMOTE_PENDING/DOWNLOADED，独立于 `lifecycleState`）、`remoteMasterPath`、`remoteThumbnailPath` + 索引；新增 DAO 查询与 `markUploaded/markDownloaded/applyRemoteTombstone`。
- **入队**：`RoomChatMediaTransactionRepository` 在绑定媒体的同一事务内为每张已发送图片入队 `UPSERT_MEDIA_ASSET`（仅新照片，不回填历史）；新增 `MediaSyncQueueContract/Writer/PayloadBuilder`，注册进 `SyncPayloadParser` 白名单与 `SyncQueueDao` 优先级。
- **推送**：`SupabaseRemoteSyncGateway.upsertMediaAsset` 先 PUT 主图+缩略图字节到私有 bucket `media-assets`（`x-upsert`，对象键 `{user_id}/{mediaId}/master.jpg|thumb.jpg`），再 upsert `media_assets` 元数据行（字节先于元数据）；`downloadMediaAsset` GET 字节写回本地。二进制 IO 经 `MediaBinaryStore`（core:data `AndroidMediaBinaryStore`）。
- **拉取**：`SupabaseMediaRemotePullGateway`（keyset 游标 `server_updated_at,id`）+ `MediaPullCoordinator` + `MediaRemoteMerger`（新行插为 REMOTE_PENDING 并入队 `DOWNLOAD_MEDIA_ASSET`；墓碑软删除；脏行延后；元数据不可变）+ `MediaPullStateStore`。接入 `InProcessSyncScheduler` 与 Hilt。
- **远端**：迁移 `20260711120000_dayzero_media_assets_sync_schema.sql` 建 `public.media_assets`（镜像 ai_chat 约定：client-UUID PK、`user_id default auth.uid()`、`server_updated_at` 触发器、owner-scoped 父 FK、RLS select/insert/update 仅 own、无 delete 策略）+ 私有 Storage bucket + `storage.objects` owner-folder RLS。已通过 MCP 应用到 DayZero 项目并跑配套 verification（17/17 passed）。
- **UI**：`MessageWithMedia` 新增 `RemotePending` 引用态，`AiRecordScreen` 为下载中图片显示 spinner+「图片下载中」。

### 隐私影响
照片自此**离开设备并持久化到 Supabase Storage**（跨设备同步的固有前提）。bucket 为私有且 RLS 按 owner 目录隔离；原 base64→Kimi 的临时视觉链路不变，Storage 持久化为新增。

### 删除策略
软删除元数据行 + 保留 Storage 字节（暂不物理删除；孤儿对象清理留待后续任务）。

### 验证
- `:core:database:testDebugUnitTest`、`:core:data:testDebugUnitTest`、`:core:sync:testDebugUnitTest`、`:app:testDebugUnitTest`、`:feature:ai-record:testDebugUnitTest` 全部 PASS；`:app:assembleDebug` BUILD SUCCESSFUL。
- 新增单测：`Migration12to13Test`、`MediaSyncPayloadBuilderTest`、`MediaSyncQueueWriterTest`、`MediaRemoteMergerTest`、`SupabaseMediaRemotePullGatewayTest`。
- Supabase 远端 schema/Storage/RLS verification 17/17 passed。
- **待真机复测**：设备 A 拍照发送 → 确认 Storage 对象+`media_assets` 行 → 设备 B 同账号 → 拉取+下载+渲染（占位→图片）。Room 版本现为 **13**。

## 阶段：应用身份统一与阿里云 App 备案准备（2026-07-12）

### 应用身份迁移

- 将 Android `applicationId` 与所有模块 `namespace` 统一为 `com.goings.dayzero`。

- 迁移范围：`app`、`core:data`、`core:database`、`core:network`、`core:sync`、`core:ui`、`feature:ai-record`、`feature:calendar`、`feature:trends`。

- 将 `com/example` 下所有 Kotlin/Java 源码目录物理移动到 `com/goings/dayzero`，共 22 个 source-set。

- 批量更新 324 个 `.kt`/`.java`/`.kts` 文件中的 `package`、`import` 与完全限定类名。

- 更新 `scripts/install-debug-preserve-data.ps1` 中的 `adb shell am start` 包名/Activity 路径。

- `AndroidManifest.xml` 使用相对名（`.MainActivity` / `.DayZeroApplication`），无需修改。

- 未修改业务逻辑、数据库表结构、Supabase 配置、AI Gateway 配置或功能行为。

### 构建与测试验证

- `./gradlew clean :app:assembleDebug` BUILD SUCCESSFUL，生成包名为 `com.goings.dayzero` 的 Debug APK。

- `./gradlew testDebugUnitTest`：98 个测试完成，3 个失败，全部集中在 `:feature:ai-record` 的 `PinnedPhotoStripUiTest`；失败原因是迁移前工作区中 `AssistantCardRenderer.kt` 已缺失照片编辑入口逻辑，与包名迁移无关。

### 正式 release APK 与备案信息

- Android Studio 生成正式 release APK：`app/release/app-release.apk`（16,131,345 字节，约 16 MB）。

- 签名方案：APK Signature Scheme v2；签名者数量 1。

- 证书 DN：`C=CN, ST=Yunnan, L=Kunming, O=DayZero, OU=DayZero, CN=Goings`（非 Android Debug）。

- 证书 MD5 指纹：`d7:4c:5d:f4:0e:d7:92:07:81:fa:dd:67:fc:a4:8a:24`。

- 阿里云备案公钥字符串（Base64 SubjectPublicKeyInfo）：

  ```

  

  ```

- 未输出 keystore 密码、私钥内容或环境变量；未重新签名、覆盖 APK 或提交 Git。

## 阶段 G1：阿里云 Gateway 本地协议补齐（2026-07-12）

### 目标

在 `server/dayzero-ai-gateway/` 完成四项本地协议补齐，不部署、不访问 ECS、不修改 Android/Supabase/Room/同步。

1. 增加正式 `/api/ai/` 路由。
2. 增加独立 `/ready` Readiness 接口。
3. 建立完整、稳定、安全的 `X-Request-Id` 契约。
4. 加固 HTTP 请求入口校验，避免非法 JSON 或错误 DTO 进入 Prompt、产生 500 或调用 Kimi。

### 修改文件

- `server/dayzero-ai-gateway/src/main.ts`
  - 导出 `handler` 供测试使用。
  - 新增 `POST /api/ai/assistant-turn-v2` 与 `POST /api/ai/assistant-turn-v2-stream`。
  - 保留旧路径 `POST /assistant-turn-v2` / `POST /assistant-turn-v2-stream` 作为兼容别名。
  - 新增 `GET /ready`；`GET /health` 保持仅进程存活检查。
  - 统一为所有响应补齐 `X-Request-Id` header。
  - 未知路径返回 404，方法不支持返回 405。
- `server/dayzero-ai-gateway/src/handlers/ready.ts`（新增）
  - 静态检查配置完整性：Kimi URL/Key/Model、可选认证启用时的 Supabase URL/Audience。
  - 不调用外部服务，不输出 Secret。
- `server/dayzero-ai-gateway/src/request_id.ts`
  - 新增 `resolveRequestId`：合法入站 ID 复用，非法/缺失时生成新 ID。
  - 安全字符集：ASCII 字母、数字、`-`、`_`、`.`、`:`，长度 8–128。
- `server/dayzero-ai-gateway/src/body_reader.ts`
  - JSON body 不是 object（null/array/string/number）时返回 `400 INVALID_BODY_TYPE`。
- `server/dayzero-ai-gateway/src/shared/request_parser.ts`
  - 校验 `turnType`、`date`、`recentMessages`、`todayRecord`、`pendingDraft`、`userText`、`interactionResult` 类型。
  - `interaction_result` 必须携带 object 类型 `interactionResult`。
  - 未知字段保留，不破坏现有 Android DTO。
- `server/dayzero-ai-gateway/src/cors.ts`
  - 已允许 `x-request-id` header 并暴露 `X-Request-Id`；未改动。

### 新增测试

- `src/main_test.ts`：11 个路由/方法/Request ID 测试。
- `src/handlers/ready_test.ts`：8 个 readiness 测试。
- `src/request_id_test.ts`：10 个 request id 解析测试。
- `src/shared/request_parser_test.ts`：13 个请求校验测试。
- `src/body_reader_test.ts`：6 个非法 body 类型测试。

### 验证结果

- `deno fmt --check`：通过（37 个文件）。
- `deno lint`：通过（34 个文件）。
- `deno check src/main.ts`：通过。
- `deno task test`：80 passed，0 failed（原 33 个）。

### 冻结文件校验

以下文件 SHA-256 与任务开始时完全一致：

- `src/shared/assistant_upstream_timeout.ts`: `9e5cd566...`
- `src/shared/prompt.ts`: `f616f8b1...`
- `src/shared/normalization.ts`: `bba2537f...`
- `src/shared/assistant_vision.ts`: `84797451...`
- `src/shared/explicit_photo_meal_assignment.ts`: `c28f3e29...`

### 范围声明

- 未修改 Android。
- 未修改 Supabase Edge Function / Auth / DB / RLS / Storage。
- 未修改 timeout、Prompt、normalization、Vision、candidate race。
- 未部署 Docker / ACR / ECS / Nginx / Android APK。
- 未调用真实 Kimi。
- 未执行 Git add / commit / push / pull / checkout / reset / clean。
