...
import com.goings.dayzero.data.sync.ChatRemotePullResult
...
import com.goings.dayzero.data.sync.RemotePullResult
import com.goings.dayzero.data.sync.SyncHealthReporter
import com.goings.dayzero.data.sync.SyncTriggerReason
import com.goings.dayzero.data.sync.chat.ChatConversationPullCoordinator
import com.goings.dayzero.data.sync.chat.ChatConversationPullStateStore
import com.goings.dayzero.data.sync.chat.ChatConversationRemoteMerger
import com.goings.dayzero.data.sync.chat.ChatMessagePullCoordinator
import com.goings.dayzero.data.sync.chat.ChatMessagePullStateStore
import com.goings.dayzero.data.sync.chat.ChatMessageRemoteMerger
import com.goings.dayzero.data.sync.chat.ChatPullCoordinator
import com.goings.dayzero.data.sync.chat.ChatPullHealthStateStore
...
class DayZeroChatSyncPullIntegrationTest {
    ...
    private class FakeChatRemotePullGateway : ChatRemotePullGateway {
        ...
        override suspend fun fetchConversationPage(
            ...
            cursor: com.goings.dayzero.domain.model.sync.ChatSyncServerCursor?,
            limit: Int
        ): ChatRemotePullResult<com.goings.dayzero.domain.model.sync.ChatRemoteConversationPage> {
            ...
            val nextCursor = items.lastOrNull()?.let {
                com.goings.dayzero.domain.model.sync.ChatSyncServerCursor(it.updatedAtMillis.toString(), it.id)
            }
            return ChatRemotePullResult.Success(
                com.goings.dayzero.domain.model.sync.ChatRemoteConversationPage(items, nextCursor, hasMore)
            )
        }
        ...
        override suspend fun fetchMessagePage(
            ...
            cursor: com.goings.dayzero.domain.model.sync.ChatSyncServerCursor?,
            limit: Int
        ): ChatRemotePullResult<com.goings.dayzero.domain.model.sync.ChatRemoteMessagePage> {
            ...
            return ChatRemotePullResult.Success(
                com.goings.dayzero.domain.model.sync.ChatRemoteMessagePage(items, nextCursor, hasMore)
            )
        }
    }
    ...
    fun `Full Pipeline Chat Sync Pull Integration`() = runTest {
        ...
        val pullCoordinator = PullCoordinator(
            database = database,
            remotePullGateway = com.goings.dayzero.data.sync.NoopRemotePullGateway(),
            identityProvider = identityProvider,
        ...
        val healthReporter = SyncHealthReporter(
            ...
            identityProvider = identityProvider,
            backfillStateStore = com.goings.dayzero.data.sync.BackfillStateStore(context),
        ...
        assertEquals(com.goings.dayzero.data.sync.PullStatus.COMPLETED, snapshot.chatPullStatus)
    }
    ...
    fun `Missing Parent Rolls Back Page and Does Not Advance Cursor`() = runTest {
        ...
        val snapshot = healthReporter.snapshot()
        assertEquals(com.goings.dayzero.data.sync.PullStatus.FAILED_RETRYABLE, snapshot.chatPullStatus)
    ...
    fun `Idempotency and Cursor Advancement - Multiple Runs Do Not Duplicate Data`() = runTest {
        ...
        val pullCoordinator = PullCoordinator(database, com.goings.dayzero.data.sync.NoopRemotePullGateway(), identityProvider, PullStateStore(context))
    ...
    fun `Confirmed Card Pull Does Not Write to DailyRecord`() = runTest {
        ...
        val healthReporter = SyncHealthReporter(syncQueueDao, identityProvider, com.goings.dayzero.data.sync.BackfillStateStore(context), PullStateStore(context), ChatPullHealthStateStore(context), database.dailyRecordDao(), { true })
        val pullCoordinator = PullCoordinator(database, com.goings.dayzero.data.sync.NoopRemotePullGateway(), identityProvider, PullStateStore(context))
