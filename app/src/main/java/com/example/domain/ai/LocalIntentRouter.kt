...
object LocalIntentRouter {
    private const val TAG = "DayZeroLocalRouter"
    ...
    fun classify(text: String): LocalUserIntent {
        ...
        Log.d(
            TAG,
            "emotionDetected=${signals.emotionDetected}, foodDetected=${signals.foodDetected}, mealTimeDetected=${signals.mealTimeDetected}"
        )

        // 1. App Commands (Clear Data)
        if (CLEAR_DATA_KEYWORDS.any { textLower.contains(it) }) {
            Log.d(TAG, "Classified: AppCommandClearData")
        ...
        if (WEIGHT_KEYWORDS.any { textLower.contains(it) }) {
            ...
            if (weight != null) {
                if (weight in 20.0..250.0) {
                    Log.d(TAG, "Classified: WeightLogging ($weight kg)")
                    return LocalUserIntent.WeightLogging(weight, text)
                }
                Log.d(TAG, "Classified: WeightLogging (Invalid value: $weight)")
        ...
        if (signals.mealTimeDetected && signals.foodDetected && 
            ...
            Log.d(TAG, "Classified: ClearFoodLogging")
            return LocalUserIntent.ClearFoodLogging(text)
        }

        // 4. Everything else goes to AI
        Log.d(TAG, "Classified: Unknown (routing to AI)")
